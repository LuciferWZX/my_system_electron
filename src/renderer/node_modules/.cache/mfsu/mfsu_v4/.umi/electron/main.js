...
const require$$1__default$2 = /* @__PURE__ */ _interopDefaultLegacy(require$$1);
const require$$0__namespace = /* @__PURE__ */ _interopNamespace(require$$0);
const require$$0__default = /* @__PURE__ */ _interopDefaultLegacy(require$$0);
const fs__namespace = /* @__PURE__ */ _interopNamespace(fs$3);
const fs__default = /* @__PURE__ */ _interopDefaultLegacy(fs$3);
...
const createProtocol = (scheme) => {
  require$$1.protocol.registerFileProtocol(scheme, (request, respond) => {
    let pathName = new url.URL(request.url).pathname;
    pathName = decodeURI(pathName);
    const filePath = require$$0__namespace.join(__dirname, pathName);
    respond({ path: filePath });
  });
};
let cacheDownItem = {};
const mainWindowIpcStart = (win) => {
  require$$1.ipcMain.on("toggle_dev_tools", function(event, arg) {
    win.webContents.toggleDevTools();
  });
  require$$1.ipcMain.on("restart", function() {
    require$$1.app.relaunch();
    require$$1.app.exit(0);
  });
  require$$1.ipcMain.handle("min", function() {
    win.minimize();
  });
  require$$1.ipcMain.handle("max", function() {
    if (win.isMaximized()) {
      win.unmaximize();
    } else {
      win.maximize();
  ...
  require$$1.ipcMain.handle("close", function() {
    cacheDownItemClose();
    win.close();
  });
  require$$1.ipcMain.handle("destroy", function() {
    win.destroy();
  });
  require$$1.ipcMain.on("set_path", function(e, data = {}) {
    let { path: path2 } = data;
    if (path2) {
      if (path2 !== "not")
        require$$1.app.setPath("downloads", path2);
      e.reply("set_path", require$$1.app.getPath("downloads"));
    } else {
      require$$1.dialog.showOpenDialog({
        title: "\u9009\u62E9\u4E0B\u8F7D\u76EE\u5F55",
        defaultPath: require$$1.app.getPath("downloads"),
        properties: ["openDirectory"]
      }).then((files) => {
        if (!files.canceled) {
          require$$1.app.setPath("downloads", files.filePaths[0]);
        }
        e.reply("set_path", files);
      });
    }
  });
  require$$1.ipcMain.on("check_path", function(e, data = {}) {
    let { path: path2 } = data;
    fs__namespace.access(path2, fs__namespace.constants.F_OK, (err) => {
      if (!err) {
        require$$1.shell.showItemInFolder(path2);
      }
      e.reply("check_path" + path2, err);
    });
  });
  require$$1.ipcMain.on("down-file", function(e, data) {
    let { url: url2 } = data;
  ...
  require$$1.ipcMain.on("down-file-pause", function(e, data) {
    let { url: url2 } = data;
    let t2 = cacheDownItem[url2];
    if (t2) {
      t2._downFileItem.pause();
    }
    e.reply("down-file-pause-" + url2, "\u5DF2\u6682\u505C");
  });
  require$$1.ipcMain.on("down-file-resume", function(e, data) {
    let { url: url2 } = data;
    let t2 = cacheDownItem[url2];
    if (t2) {
      t2._downFileItem.resume();
    }
    e.reply("down-file-resume-" + url2, "\u5DF2\u6062\u590D\u4E0B\u8F7D");
  });
  require$$1.ipcMain.on("down-file-cancel", function(e, data) {
    let { url: url2 } = data;
    let t2 = cacheDownItem[url2];
    if (t2) {
      t2._downFileItem.cancel();
    }
    e.reply("down-file-cancel-" + url2, "\u5DF2\u53D6\u6D88\u4E0B\u8F7D");
  });
  require$$1.ipcMain.on("resume-download", function(e, data) {
    let { url: url2 } = data;
    let t2 = cacheDownItem[url2];
    if (t2) {
      t2._downFileItem.resume();
  ...
  const downFile = (url2) => {
    require$$1.session.defaultSession.downloadURL(url2);
  };
  const resumeDownload = (obj = {}) => {
    ...
    require$$1.session.defaultSession.createInterruptedDownload({
      path: path2,
      urlChain,
      offset,
      length,
  ...
  require$$1.session.defaultSession.on("will-download", (e, item) => {
    try {
      ...
      const filePath = require$$0__namespace.join(require$$1.app.getPath("downloads"), item.getFilename());
  ...
  require$$1.app.on("gpu-process-crashed", function() {
    cacheDownItemClose();
  });
  require$$1.app.on("renderer-process-crashed", function() {
    cacheDownItemClose();
  });
  let noti;
  const notification = (url2) => {
    noti = new require$$1.Notification({
      title: "\u4E0B\u8F7D\u6210\u529F",
      body: url2,
      silent: false,
      icon: url2
    ...
    noti.show();
    noti.once("click", () => {
      require$$1.shell.showItemInFolder(url2);
    });
  };
};
...
const storeIpcStart = (store) => {
  require$$1.ipcMain.handle("get_store", async (event, key) => {
    return store.get(key);
  });
  require$$1.ipcMain.handle("set_store", async (event, key, value) => {
    return store.set(key, value);
  });
  require$$1.ipcMain.handle(
    "update_store",
    async (event, storeKey, key, newData) => {
...
if (!gotTheLock) {
  ...
  const createWindow = (store) => {
    ...
    win.on("close", (event) => {
      const showConfirmType = store.get("showConfirmTypeModal");
      const closeType = store.get("closeType");
  ...
  require$$1.app.on("ready", () => {
    console.log("\u8DEF\u5F84\uFF1A", require$$1.app.getPath("userData"));
    let option = {
      name: "big_tool_config",
      fileExtension: "json",
      cwd: require$$1.app.getPath("userData"),
      clearInvalidConfig: true
    };
    const store = new electronStore(option);
  ...
  require$$1.app.on("quit", () => {
    console.log("quit:");
    require$$1.app.releaseSingleInstanceLock();
  });
}
